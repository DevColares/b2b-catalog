    ...
    <div className="min-h-screen pb-20 transition-colors duration-300" style={{background: isDarkMode ? 'linear-gradient(135deg, #12030d 0%, #1e0918 30%, #17041c 60%, #1c051a 100%)' : 'linear-gradient(135deg, #FDF7F7 0%, #F5E6E8 50%, #EBD5D8 100%)'}}>
        ...
        <div className="max-w-6xl mx-auto px-3 sm:px-4 h-16 sm:h-20 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="relative w-10 h-10 sm:w-11 sm:h-11 flex items-center justify-center flex-shrink-0">
              <div className="absolute inset-0 rounded-2xl glass" style={{background: 'linear-gradient(135deg, #F2D6DC, #C48B96)'}}></div>
              <span className="relative text-xl sm:text-2xl select-none">🌸</span>
            </div>
            <div className="min-w-0">
              <h1 className="font-black text-base sm:text-lg leading-none truncate" style={{color: isDarkMode ? '#fbcfe8' : '#8C4A5A', fontFamily: "'Plus Jakarta Sans', sans-serif", letterSpacing: '-0.5px'}}>Super Ketlen Mayluce</h1>
              <p className="hidden sm:block text-xs font-semibold mt-1" style={{color: isDarkMode ? '#f472b6' : '#C48B96', letterSpacing: '0.05em'}}>CATÁLOGO EXCLUSIVO</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {totalCartItems > 0 && (
                <span className="absolute -top-2 -right-2 w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold border-2 border-white shadow-sm" style={{background:'#8C4A5A', color:'#fff'}}>
                  {totalCartItems}
                </span>
              )}
            </button>
      ...
      <div className="py-10 sm:py-14 px-4 mb-8 relative overflow-hidden" style={{background: 'linear-gradient(135deg, rgba(242,214,220,0.55) 0%, rgba(234,205,213,0.4) 40%, rgba(196,139,150,0.45) 100%)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)'}}>
        ...
        <div className="max-w-3xl mx-auto text-center relative">
          <p className="text-primary text-xs sm:text-sm font-semibold tracking-widest mb-2 uppercase">✨ Bem-vinda ✨</p>
      ...
      <main className="max-w-6xl mx-auto px-4">
        ...
        {featuredProducts.length > 0 && !searchQuery && (
          <div className="mb-10">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-xl font-black flex items-center gap-2" style={{color: isDarkMode ? '#fce7f3' : '#2d1e23'}}>
        ...
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold" style={{color: isDarkMode ? '#fce7f3' : '#2d1e23'}}>
            {searchQuery ? 'Resultados da Busca' : 'Todos os Produtos'}
          </h3>
          <span className="text-sm font-medium" style={{color: isDarkMode ? '#f472b6' : '#9e8b92'}}>
        ...
        {filteredProducts.length > 0 ? (
          ...
          <div className="text-center py-20" style={{color: isDarkMode ? '#f472b6' : '#9e8b92'}}>
      ...
      {toast && (
        <div 
          className="toast-pop fixed bottom-6 left-1/2 z-[60] w-[calc(100%-2rem)] max-w-md"
          role="status"
          aria-live="polite"
        >
          <div 
            ...
            style={{
              background: 'rgba(253,247,247,0.85)',
              backdropFilter: 'blur(16px) saturate(180%)',
              WebkitBackdropFilter: 'blur(16px) saturate(180%)',
              borderColor: toast.type === 'success' ? 'rgba(140,74,90,0.15)' : 'rgba(180,60,60,0.25)',
              boxShadow: '0 12px 40px 0 rgba(140,74,90,0.2)'
            }}
          >
            {toast.type === 'success' ? (
              <CheckCircle2 size={24} className="flex-shrink-0 mt-0.5" style={{color: '#8C4A5A'}} />
            ) : (
              <XCircle size={24} className="flex-shrink-0 mt-0.5" style={{color: '#B43C3C'}} />
            )}
            <div className="flex-1">
              <p className="font-bold text-sm" style={{color: toast.type === 'success' ? '#8C4A5A' : '#B43C3C'}}>
